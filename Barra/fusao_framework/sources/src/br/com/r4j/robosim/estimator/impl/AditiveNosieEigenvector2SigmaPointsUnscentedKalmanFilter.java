...
public class AditiveNosieEigenvector2SigmaPointsUnscentedKalmanFilter extends AditiveNosieUnscentedKalmanFilter
{
	private static Log log = LogFactory.getLog(AditiveNosieEigenvector2SigmaPointsUnscentedKalmanFilter.class.getName());
	private static Log logEigenUKF = LogFactory.getLog("eigenukf");

	
	public AditiveNosieEigenvector2SigmaPointsUnscentedKalmanFilter()
	{
		super();
	...
	protected AbstractDoubleMatrix generateSigmaPoints(AbstractDoubleVector mean, AbstractDoubleSquareMatrix covar)
	{
		...
		for (int idxObj = 0; idxObj < 3 && idxObj < mean.dimension(); idxObj++) for (int idx = 0; idx < mean.dimension(); idx++)
		{
			sigma.setElement(idx, idxObj + 1, zMean.getComponent(idx));
			sigma.setElement(idx, idxObj + 1 + mean.dimension(), zMean.getComponent(idx));
		}

		double sigXd2 = gamma*Math.sqrt(usv[1].getElement(0, 0))/2;
		double sigYd2 = gamma*Math.sqrt(usv[1].getElement(1, 1))/2;
		double sigTheta = gamma*Math.sqrt(usv[1].getElement(2, 2)); 

		sigma.setElement(0, 0 + 1, zMean.getComponent(0) + sigXd2);
		sigma.setElement(0, 1 + 1, zMean.getComponent(0) - sigXd2);
		sigma.setElement(0, 0 + 1 + mean.dimension(), zMean.getComponent(0) + sigXd2);
		sigma.setElement(0, 1 + 1 + mean.dimension(), zMean.getComponent(0) - sigXd2);
		sigma.setElement(1, 0 + 1, zMean.getComponent(1) + sigYd2);
		sigma.setElement(1, 1 + 1, zMean.getComponent(1) + sigYd2);
		sigma.setElement(1, 0 + 1 + mean.dimension(), zMean.getComponent(1) - sigYd2);
		sigma.setElement(1, 1 + 1 + mean.dimension(), zMean.getComponent(1) - sigYd2);

		sigma.setElement(2, 2 + 1, zMean.getComponent(2) + sigTheta);
		sigma.setElement(2, 2 + 1 + mean.dimension(), zMean.getComponent(2) - sigTheta);
	...
	{
		return new Eigen2UKFModelFilter();
	}
}


class Eigen2UKFModelFilter implements ModelFilter
{
