...
/*
		this.loadSensorModelMangers(props, props.getSubGroupOfKeys(strPathBase + "sim_world/sensormanangers", 1), props.getSubGroupOfKeys(strPathBase + "sim_world/sensor_models", 1), props.getSubGroupOfKeys(strPathBase + "sim_world/dynamic_models", 1), listSimSensorsInfo, listSimSensorModelsInfo, listSimDynModelInfo, mapSimSensorId2SensorInfo);
		this.loadSensorModelMangers(props, props.getSubGroupOfKeys(strPathBase + "real_world/sensormanangers", 1), props.getSubGroupOfKeys(strPathBase + "sim_world/sensor_models", 1), props.getSubGroupOfKeys(strPathBase + "sim_world/dynamic_models", 1), listSimSensorsInfo, listSimSensorModelsInfo, listSimDynModelInfo, mapSimSensorId2SensorInfo);
//*/
	}
...
//			strId = strId.substring(strId.lastIndexOf("/") + 1, strId.length());
		...
		while (itProps.hasNext())
		{
			...
			Object oSensModel = props.getObjectProperty(strProp);
			if (oSensModel instanceof SensorModel)
			{
				SensorModel sensModel = (SensorModel) oSensModel;
			...
			{
				String sensModel = (String) oSensModel;
				ImplSensorModelInfo sensModelInfo = new ImplSensorModelInfo(props.getStringProperty(strProp + "/name"));
				if (props.containsProperty(strProp + "/inverse_model_avail"))
					sensModelInfo.setUsesInvertedModel(props.getBooleanProperty(strProp + "/inverse_model_avail").booleanValue());
				else
...
//*/
	...
	public Estimator createEstimator(EstimationComponentsInfo estCompsInfo) throws ConfiguratorException
	{
		...
		String strEstInfo = estCompsInfo.getName();

		Iterator itSensModels = estCompsInfo.getSensorModels().iterator();
		if (itSensModels.hasNext())
			strEstInfo += ":";
		while (itSensModels.hasNext())
		{
			...
			log.debug("SensorModel snsMdl = sensModel.getSensorModel(): " + snsMdl);
			if (snsMdl != null)
			{
				Sensor sns = ((ImplSensorInfo) sensModel.getSensorInfo()).getSensor();
				snsMdl.setSensor(sns);
				est.addSensorModel(snsMdl, sns);
				strEstInfo += snsMdl.getName() + ", ";
				log.debug("sensModel.getName(): " + sensModel.getName());
				log.debug("sensModel.getSensorInfo().getName(): " + sensModel.getSensorInfo().getName());
				log.debug("sensModel.getName(): " + sensModel.getName());
			}
		}

		if (est instanceof BaseEstimator)
			((BaseEstimator) est).setInstanceName(strEstInfo);
