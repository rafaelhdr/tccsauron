...
O filtro de Kalman estendido (EKF) é um filtro matemático capaz de realizar a previsão e correção das estimativas de postura. As equações para cada uma das etapas podem ser vistas em \ref{table:kalman}, e são executadas na ordem da tabela. O EKF prevê a linearização do sistema em sua borda a fim de que o filtro de kalman possa ser utilizado com sucesso. Como o método de linearização utilizado no EKF é a expansão de taylor de primeira ordem, são geradas as matrizes $F$, jacobiano de $f(\textbf{x},\textbf{u})$, e $H$, jacobiano de $h(\textbf{x})$. 
...
\begin{tabular}{c c }
\hline \hline
\textbf{Predição} &  \textbf{Correção} \\ [0.5ex]
\hline
$\bar{\textbf{x}} = f(\textbf{x}_{t-1},\textbf{u}_{t-1})$ & $K_{t} = P_{t}^{-}H_{t}^{T}(H_{t}P_{t}^{-}H_{t}^{T}+R_{t})^{-1}$ \\
$P_{t}^{-} = F_{t-1}P_{t-1}^{+}F_{k-1}^{T} + Q_{t-1}$ & $\hat{\textbf{x}}_{t}^{+} = \hat{\textbf{x}}_{t}^{-} + K_{t}(\textbf{z}_{t}-h(\textbf{x}_{t}^{-}))$ \\
& $P_{t}^{+} = (I - K_{t}H_{t})P_{t}^{-}$
\end{tabular}
\label{table:kalman}
\end{table}

Por fim, o último componente do sistema é o módulo de coordenação, que liga os demais componentes do sistema de localização, iniciando os sensores e associando-os ao EKF.
...
\todo{colocar aqui embaixo a figura bem geral do kalman, Predição (modelo de dinâmica) gera x barra para a Correção (modelos de observação), que recebe x barra e alimenta com um atraso para o modelo de dinamica - A camera e os sonares conversam com a Correção. O odômetro conversa com a Predição.}
...
\todo{mudar figura abaixo puxando o odômetro para cima e criando um quadrado chamado Localizacao, e trocar o que eh localizacao por Gerenciador de Localização}
