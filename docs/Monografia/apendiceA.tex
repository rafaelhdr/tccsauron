...
\begin{gather}
	\textbf{x}_{k}=F_{k-1}\textbf{x}_{k-1}+G_{k-1}\textbf{u}_{k-1}+\textbf{w}_{k-1} \nonumber \\
	\textbf{y}_{k}=H_{k}\textbf{x}_{k}+\textbf{v}_{k}
\end{gather}
onde os ruídos $\textbf{w}_{k-1}$ e $\textbf{v}_{k}$ são brancos, centrados no zero, não correlatos e possuem matrizes de covariância $Q$ e $R$ respectivamente.

Para estimar o valor de $\textbf{x}_{k}$ divide-se $\textbf{x}_{k}$ em dois valores distintos, $\hat{\textbf{x}}_{k}^{-}$ e $\hat{\textbf{x}}_{k}^{+}$, onde o primeiro corresponde a um valor antes de uma observação e o segundo após a observação, i.e., $\hat{\textbf{x}}_{k}^{-}$ corresponde ao valor estimado baseado na dinâmica do sistema em um instante $k$ instantâneamente antes de uma medição ser realizada, enquanto que $\hat{\textbf{x}}_{k}^{+}$ corresponde ao valor estimado no mesmo instante $k$ após a obtenção de uma estimativa $\textbf{y}_{k}$. De forma semelhante, $P_{k}$ é dividido em $P_{k}^{-}$ e $P_{k}^{+}$, sendo estas as matrizes de covariâncias antes e depois de uma observação no instante $k$ respectivamente.
...
\begin{itemize}
	\item Descreve-se matematicamente o sistema:
		\begin{gather}
			\textbf{x}_{k}=F_{k-1}\textbf{x}_{k-1}+B_{k-1}\textbf{u}_{k-1}+\textbf{w}_{k-1} \nonumber \\
			\textbf{y}_{k}=H_{k}\textbf{x}_{k}+\textbf{v}_{k} \nonumber \\
			E(\textbf{w}_{k}\textbf{w}^{T}_{j})=Q_{k}\delta_{k-j} \\
			E(\textbf{v}_{k}\textbf{v}^{T}_{j})=R_{k}\delta_{k-j} \nonumber \\
			E(\textbf{w}_{k}\textbf{v}^{T}_{j})=0 \nonumber 
		\end{gather}	
	...
	\item Itera-se o filtro para cada uma das seguintes equações:
		\begin{gather}
			...
			\bar{\textbf{x}}^{-}_{k}=F_{k-1}\hat{\textbf{x}}^{+}_{k-1}+B_{k-1}\textbf{u}_{k-1} \nonumber \\
			K_{k}=P^{-}_{k}H^{T}_{k}(H_{k}P^{-}_{k}H^{T}_{k}+R_{k})^{-1} \\
			\hat{\textbf{x}}^{+}_{k}=\bar{\textbf{x}}^{-}_{k}+K_{k}(\textbf{y}_{k}-H_{k}\bar{\textbf{x}}^{-}_{k}) \nonumber \\
