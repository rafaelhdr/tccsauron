...
\begin{equation}
	y = Hx + v
\end{equation}
Onde \bf{y} é um vetor com as medições, \bf{x} é a constante a ser determinada, mas que também se apresenta em forma de vetor (iguais) e \bf{v} é o ruído da medição.
...
\[
	\dot{x}=Ax+Bu+w
\]
Onde $\bf{x}$ é o estado do sistema, $\bf{u}$ é uma variável de controle e $\bf{w}$ é o ruído do sistema, com média nula e com distribuição gaussiana. Por ser uma equação diferencial, sabemos que a solução do sistema acima para um determinado tempo $t_{k}$ é:
\begin{equation}
