...
O módulo de navegação têm conhecimento prévio do mapa do ambiente. Este mapa é marcado com alguns pontos, permitindo a formação de um grafo e utilização de algoritmos de busca em grafos para planejamento da rota.
...
Um problema que logo se manifestou para a navegação era o cuidado especial com retrocessos. Algumas vezes, é necessário  afastar-se um pouco do destino antes de poder alcançá-lo.  A figura \ref{navegacao:mapaObstaculo} abaixo ilustra essa situação. Para ir do ponto A ao ponto D, deve-se contornar a parede, passando por exemplo, pelo ponto B, que está mais distante do destino D do que o próprio A.

\begin{figure}[h]%
\begin{center}
\includegraphics[width=.5\columnwidth]{imagens/mapaObstaculo.jpg}
\caption{Mapa que ilustra um problema comum a ser resolvido pela Navegação}%
...
Para isso, carrega-se o sistema com todos os mapas disponíveis e cria-se ligações entre os mapas através de \textit{waypoints} especiais chamados portais. De fato, basta que um waypoint em um mapa tenha o mesmo nome de outro waypoint em outro mapa para que seja estabelecido um portal entre eles.
...
Para determinação da rota entre mapas utilizou-se um simples algoritmo de Busca em Profundidade aplicado no grafo da relação entre os mapas. Decidiu-se por esse algoritmo pela sua simplicidade e por saber-se que o grafo de mapas nunca seria muito extenso.

Um último ponto que deve ser ressaltado é que, ao realizar a troca de mapas, a postura do robô deve ser atualizada para o sistema de coordenadas do novo mapa, este é um ponto interessante e que pode passar despercebido.

Assim, a arquitetura final do módulo de navegação ficou como abaixo representada na figura \ref{navegacao:arquitetura}, constando dos Planejamentos Inter Mapa e Planejamento 
