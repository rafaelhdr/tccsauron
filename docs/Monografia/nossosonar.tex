...
\chapter{Modelos de Observação dos Sonares}\label{chp:modelosonares}
...
\label{arquitetura:globalsonar}%
\end{center}
\end{figure}

A figura \ref{arquitetura:globalsonar} apresenta novamente a arquitetura de software do sistema. O modelo de observação dos sonares, que utiliza informações dos sonares, é parte importante do módulo de localização, que provê estimativas de postura ao módulo de navegação.

\section{Modelo Baseado em Associações}
\label{sec:nossosonar}
A seção \ref{subsec:sonarbarra} apresentou o modelo de observação desenvolvido em \cite{barra}. O modelo de observação baseado em associações, aqui apresentado, é uma modificação desse modelo realizada pela equipe e caracteriza-se como uma contribuição deste trabalho. Em particular, as equações foram refeitas para incluir o efeito do ângulo relativo de cada sonar em relação ao centro do robô, e a manutenção de associações já realizadas foi facilitada através do método de rastreamento de paredes.
...
O modelo simples de observação dos sonares é uma realização exclusiva da equipe deste trabalho e caracteriza-se, portanto, como uma contribuição deste trabalho.
