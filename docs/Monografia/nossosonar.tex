...
\subsection{Modelos de Observação dos Sonares}\label{subsec:modelosonares}
...
\subsubsection{Validação das Observações}
\label{sec:validacao}
...
\subsubsection{Associação das observações com o mapa}\label{sec:associacao}
...
As paredes que passarem por ambos os filtros são candidatas a associação. Para que se possa selecionar a parede correta, é calculada a observação esperada para aquela parede, dado que acreditamos estar em uma postura $\hat{x}$. Dá-se a essa observação esperada o nome de \hx, e seu cálculo é explicado na seção \ref{sec:obsesperada}. Averigua-se, então, a diferença entre \hx e $z$, a observação mais recente obtida do sonar. A ideia é rejeitar associações que produzam erros grandes demais.
...
\subsubsection{Rastreamento de associação}
...
\begin{equation*}
	erro_i = (z_i-h(\hat{x}_i))^2
\end{equation*}
\begin{equation*}
	\Delta_{erro} = erro_{i-1} - erro_i
\end{equation*}
\begin{equation}
	p_i = p_{i-1} + K\Delta_{erro},
\end{equation}
...
onde $\beta$ é o ângulo de incidência do feixe do sonar com a parede. Seja $(x', y', \theta')$ a postura relativa do sonar, em relação ao centro do robô. Então $\beta = \theta' + \alpha$, onde $\alpha$ é o ângulo formado pela trajetória do robô e a parede sendo observada. Como $\alpha= \pi/2 - \theta_{wall} + \theta$, tem-se:

\begin{equation}
\beta=\theta'+ \frac{\pi}{2} - \theta_{wall} + \theta
\label{eq:beta}
\end{equation}
...
\subsection{Modelo Simples de Observação dos Sonares}\label{subsec:modeloSimplesSonar}
...
\subsubsection{Determinação da parede e observação esperada}
...
\subsubsection{Obtenção da matriz de covariância e do jacobiano}
...
\subsubsection{Determinação dos parâmetros do modelo simples}
