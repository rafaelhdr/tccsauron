...
Esse modelo é mais simples, pois não leva em consideração todos os sinais de controle do robô e os detalhes de sua operação interna. Dessa forma, ele apresenta menor possibilidade de erros, mas também menor acurácia na predição.
...
Os valores de deslocamento médio linear e angular são os valores fornecidos pelo \textit{firmware} de interface com o robô. A modelagem das equações de dinâmica foi feita com base nesses valores. 
...
Essas equações formam o vetor \textbf{f} de predição do Filtro de Kalman estendido. A matriz F de propagação de incertezas será o jacobiano desse vetor, e está representada abaixo.
...
\sigma^{2}_{d_{m}}(t) cos^2(\theta_{m}(t))  & \sigma^{2}_{d_{m}}(t) cos(\theta_{m}(t)) sin(\theta_{m}(t)) & 0 \\
\sigma^{2}_{d_{m}}(t) cos(\theta_{m}(t)) sin(\theta_{m}(t)) & \sigma^{2}_{d_{m}}(t) sin^2(\theta_{m}(t)) & 0 \\
0 & 0 & \sigma^{2}_{\theta_{m}}(t) \end{array} \right) 
\end{equation}
...
\begin{gather}
	...
	\sigma^{2}_{\theta_{m}}(t) = (d_{m}(t) \sigma^{\delta\theta}_{\delta\theta})^2 + (\delta\theta(t) \sigma^{d_{m}}_{\delta\theta})^2
\end{gather}