...
\missingfigure{Figura do modelo pinhole}
...
onde ${\bf R}$ é o parâmetro de rotação relativa entre o sistema de coordenadas da câmera e o global, enquanto ${\bf C}$ é a translação entre esses sistemas de coordenadas.
...
Ele considera uma região da imagem centrada na sequência de pixels que pertencem a uma projeção identificada, estendendo-se por uma faixa de \textit{n} pixels para a direita e para a esquerda de cada pixel da projeção.
...
\begin{figure}[ht]
	...
	\caption{Projeções detectadas a partir da imagem mostrada na Figura ~\ref{visao:original}}
...
Tendo-se determinado o ângulo $alfa_{cam}$ relativo a abertura do campo de visão da câmera, na horizontal, traça-se um setor circular de raio infinito, centrado na posição $\left[x_{robo}, y_{robo}\right]^T$, apontando para a mesma direção indicada por $\theta_{robo}$, com ângulo igual a $alfa_{cam}$. Em seguida, é verificado se o ponto ${\bf p_{marco}} = \left[x_{marco}, y_{marco}\right]^T$ de cada marco encontra-se dentro desse setor circular. 
...
Após a fase de associações, tem-se a lista de marcos que estão sendo observados e que devem ser usados para atualizar a postura do robô. O próximo passo é fornecer ao filtro estendido de Kalman as informações de postura estimada ${\bf x}$, a observação real obtida ${\bf z}$, a matriz de covariância $\bm{R}$, e o jacobiano de ${\bf h}$, a matriz $\bm{H}$.

\paragraph{Cálculo da observação esperada}

Seja a postura do robô descrita como ${\bf p_{robo}} = \left[ x_{robo}, y_{robo}, \theta_{robô}\right]^T$ e ${\bf p^{(i)}_{marco}} = \left[ x^{(i)}_{marco}, y^{(i)}_{marco}\right]^T$ as coordenadas de cada marco \textit{i} observado, tem-se:

\begin{equation*}
h^{(i)}({\bf p_{robo}}) = -f_u \frac{ \sin(\theta_{robo})\left(x^{(i)}_{marco} - x_{robo} \right) + \cos( \theta_{robo}) \left( y^{(i)}_{marco} - y_{robo} \right) } { \cos(\theta_{robo}) \left(x^{(i)}_{marco} - x_{robo} \right) - \sin(\theta_{robo}) \left( y^{(i)}_{marco} - y_{robo} \right) } + u_0 
\end{equation*}

onde $f_u$ é a distância focal na direção horizontal enquanto $u_0$ é o centro horizontal do plano de projeção da câmera.
...
\bm{H} = 
	\left(
    	\begin{array}{>{\displaystyle}c}
    		f_u \frac{ \sin(\theta_{robo})Z - \cos(\theta_{robo})V}{Z^2} \\
    		\\
    		f_u \frac{ \cos(\theta_{robo})Z + \sin(\theta_{robo})V}{Z^2} \\
    		\\
...
\begin{equation*}
	V = \sin(\theta_{robo})\left(x^{(i)}_{marco} - x_{robo} \right) + \cos( \theta_{robo}) \left( y^{(i)}_{marco} - y_{robo} \right)
\end{equation*}

\begin{equation*}
	Z = \cos(\theta_{robo}) \left(x^{(i)}_{marco} - x_{robo} \right) - \sin(\theta_{robo}) \left( y^{(i)}_{marco} - y_{robo} \right)	
\end{equation*}
...
%\subsubsection{Rastreamento e Associação}
