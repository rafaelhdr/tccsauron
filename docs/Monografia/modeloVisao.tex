\chapter{Modelo de Observação da Visão}\label{chp:modeloVisao}
...
O modelo aqui apresentado é uma modificação do modelo descrito em \cite{barra}, e também caracteriza-se por uma contribuição deste trabalho. 
...
\begin{figure}%
\begin{center}
\includegraphics[width=.5\columnwidth]{imagens/arquiteturaGeral.pdf}
\caption{Arquitetura do sistema. Este capítulo trata de uma parte do módulo de localização, o modelo de de observação dos sonares. Os sonares, além de alimentar o módulo de localização, também fornecem dados ao submódulo de execução. Essa relação não é discutida neste capítulo.}%
\label{arquitetura:globalvisao}%
\end{center}
\end{figure}

A arquitetura de software do sistema é ilustrada na figura \ref{arquitetura:globalvisao}. Utilizando informações de uma câmera, o modelo de observação da visão, que compõe o módulo de localização, é capaz de gerar correções de postura.

\section{Modelo da Câmera}
Dada a característica de baixo custo desejada, a câmera usada é uma \textit{webcam} convencional, baseada na tecnologia CCD. O modelo da câmera segue o modelo de câmera de orifício, também conhecido como \textit{pinhole}. Nesse modelo, a câmera possui um centro focal C e um plano de projeção, onde as imagens são formadas, representado como um espaço discreto cujo tamanho indica a resolução da câmera e em que cada elemento discreto é um pixel da imagem. 

\begin{figure}[htbp]
	\centering
		\includegraphics{imagens/pinhole.jpg}
	\caption{Modelo de câmera pinhole, composto pelo centro focal C e pelo plano de projeção \cite{barra}.}
	\label{fig:pinhole}
\end{figure}
...
onde $f_u$ e $f_v$ indicam a distância focal, horizontal e vertical, da câmera, respectivamente, cujos valores são obtidos em uma etapa de calibração, assim como os valores de $u_0$ e $v_0$, que indicam o centro do plano de projeção, no sistema de coordenadas do plano de projeção $\left(U, V\right)$. Os valores de $u$ e $v$ são dados em pixels. 

Já um ponto no sistema de coordenadas global, isto é, no mesmo sistema de coordenadas na qual a postura do robô é representada, dado por ${\bf p_{real}} = \left[x, y, z\right]^T$, precisa ser previamente convertido para o sistema de coordenadas intrínseco à câmera, resultando em um ponto ${\bf p_{cam}} = \left[ x_{cam}, y_{cam}, z_{cam} \right]^T$, antes que as equações acima possam ser aplicadas. Tal conversão é feita por meio de:

\begin{equation}
	{\bf p_{cam}} = {\bf R^T\left( p_{real} - C_{trans} \right)}
\end{equation}
	
onde ${\bf R}$ é o parâmetro de rotação relativa entre o sistema de coordenadas da câmera e o global, enquanto ${\bf C}$ é a translação entre esses sistemas de coordenadas. Assim como $f_u$ e $f_v$, ${\bf R}$ e ${\bf C_{trans}}$ também são determinados durante a calibração da câmera.
	

\section{Realce de Projeções Verticais}

A primeira etapa do processamento da visão é realçar, nos quadros obtidos pela câmera, apenas os pixels que possam pertencer a projeções de retas verticais. Isso é feito através do uso de um Operador de Sobel modificado.

O Operador de Sobel é um algoritmo voltado para a detecção de bordas em imagens, cujo resultado é uma imagem em tons de cinza onde os pixels que representam bordas de objetos ficam demarcados em branco, enquanto os demais assumem a tons próximos ao preto. Como exemplo, o resultado da aplicação do Operador de Sobel na figura \ref{visao:original} está ilustrado na figura \ref{visao:convolucao}.
...
É importante salientar que, antes do Operador de Sobel ser aplicado, a imagem sofre uma operação de borramento para reduzir a influência que o ruído presente na captura das imagens possa ter sobre a qualidade do resultado do Operador de Sobel.
...
\section{Perfil de Cor}

A hipótese que a distribuição de cores ao redor das retas verticais contém informação suficiente para ajudar a identificar projeções de uma mesma reta vertical em diferentes quadros do vídeo dá origem ao cálculo do filtro de cor.

Considera-se uma região da imagem centrada na sequência de pixels que pertencem a uma projeção, estendendo-se por uma faixa de $n$ pixels à direita e à esquerda de cada pixel da projeção.

As informações que se mostram relevantes sobre o perfil de cor são os valores médios para cada componente de cor, denominados $R_{esq}$, $R_{dir}$, $G_{esq}$, $G_{dir}$, $B_{esq}$ e $B_{dir}$ dos lados esquerdo e direito em relação ao centro da região.
...
\end{eqnarray*}
\begin{equation}
C_{corr} = \frac{\left( R + G + B \right)}{6\delta} 
\label{eqn:perfilcor}
\end{equation}

onde $\delta$ é a maior diferença esperada entre os componentes, $R$, $G$ e $B$ são representam as diferenças nas intensidades dos componentes de cor entre o lado direito e esquerdo, em relação à reta considerada, e $C_{corr}$ é o fator de correlação. 


\section{Detecção de Projeções Verticais}
...
Ao final, cada projeção será representada a partir da posição no eixo horizontal do plano de projeção da câmera (coordenada $u_{vert}$) e seu respectivo perfil de cor.
...
Os marcos são usados, primeiramente, para que se tente estabelecer um vínculo entre projeções que estão sendo observadas com retas verticais do ambiente. Os marcos com os quais foi possível realizar uma associação são usados para atualizar a estimativa da postura do robô, conforme será mostrado posteriormente.


\section{Associação entre Projeções Verticais e Marcos}
...
Para reduzir o conjunto de marcos com os quais tentar-se-á estabelecer novas associações, e com isso, diminuir a chance de ocorrerem falsos positivos, o conjunto de todos os marcos conhecidos do ambiente passa por um filtro que, usando a última postura predita do robô, calcula quais marcos estão dentro do campo de visão da câmera. 

Tendo-se determinado o ângulo $\alpha_{cam}$ relativo à abertura do campo de visão da câmera, na horizontal, traça-se um setor circular de raio infinito, centrado nas coordenadas $\left[x, y\right]^T$ da postura do robô, apontando para a mesma direção indicada por $\theta$, com ângulo do setor igual a $\alpha_{cam}$. Em seguida, é verificado se o ponto ${\bf p_{marco}} = \left[x_{marco}, y_{marco}\right]^T$ de cada marco encontra-se dentro desse setor circular. 

É importante observar que o filtro não remove marcos que possam estar ocluídos, já que o filtro não tem informações suficientes para tal procedimento. Também não é levada em consideração a distância entre o robô e o marco como um parâmetro de restrição, pois, apesar da probabilidade de uma associação ser concretizada diminuir conforme essa distância aumenta, ela não pode ser completamente descartada neste passo inicial.


\paragraph{Janela de Busca por Previsão}
Novamente, a partir da última postura predita do robô, é possível prever a coordenada $u_{previsao}$ que a projeção vertical de um marco deveria estar localizada na imagem, usando a equação \ref{coordu:pinhole} do modelo de câmera de orifício. 
...
\section{Obtenção da Observação Esperada}

Após a fase de associações, tem-se a lista de marcos que estão sendo observados e que devem ser usados para atualizar a postura do robô. O próximo passo é fornecer ao EKF as informações de observação real obtida ${\bf z}$, os valores esperados para as observações ${\bf h}$, a matriz de covariância R, e o jacobiano de ${\bf h}$, a matriz H.
...
O vetor ${\bf x}$ é composto pelo resultado da equação acima, aplicada sobre cada um dos marcos observados, tendo dimensão $1 \times i$. 
...
Cada linha da matriz H diz respeito a um marco observado, devendo ter o número de linhas igual ao número de marcos. 

\paragraph{Cálculo da matriz de covariância} A matriz Ré uma matriz quadrada que contém as covariâncias que são passadas para o EKF durante a etapa de atualização da estimativa. Sendo $n$ o número de marcos observadas em um determinado ciclo da visão, a matriz terá dimensão $n \times n$, com os seguintes valores:
