 ...
 *
 */
public class RandomNoiseRMSStatsUI implements BlockUI, StatsComponent, EstimatorEngineListener
{
	private static Log log = LogFactory.getLog(RandomNoiseRMSStatsUI.class.getName());
...
//	private AbstractDoubleSquareMatrix [][] arrayCovars = null;
	...
	public RandomNoiseRMSStatsUI(StatsUI statsUI)
	{
		log.debug("RandomNoiseRMSStatsUI");
	...
	{
		return "Exibe o RMS de cada estimador para cada iteração aumentando o ruido cada vez";
	}
...
//		this.showLegends();
	...
	public void estimationIterationPerformed(AbstractDoubleVector [] arrayRealPoses, AbstractDoubleVector [][] arrayMeans, AbstractDoubleSquareMatrix [][] arrayCovars, int itCount, int estCount, int stepCount)
	{
		...
		if (arrayRMSMeansPerComponent == null || 
			...
			arrayRMSMeansPerComponent[0].length != rndLevelCount ||
			arrayRMSMeansPerComponent[0][0].length != stepCount ||
			arrayRMSMeansPerComponent[0][0][0].length != countComponents)
		{
			arrayRMSMeansPerComponent = new double[estCount][rndLevelCount][stepCount][countComponents];
			arrayRMSMeansTotal = new double[estCount][rndLevelCount][stepCount];
			arrayRMSMeansTotalMean = new double[estCount][rndLevelCount];
		}
		...
		for (int idxEstimator = 0; idxEstimator < estCount; idxEstimator++)
		{
			for (int idxStep = 0; idxStep < stepCount; idxStep++)
			{
				for (int idxComponent = 0; idxComponent < countComponents; idxComponent++)
				{
					...
					arrayRMSMeansPerComponent[idxEstimator][currentRndLevelIdx][idxStep][idxComponent] += Math.sqrt((valTmp - valReal)*(valTmp - valReal));
				}
			}
		}
		...
		if (holderBaseFileEstimatorResultsPerIt.get() != null)
		{
			try
			{
				...
				for (int idxEstimator = 0; idxEstimator < estCount; idxEstimator++)
				{
					FileOutputStream flOut = new FileOutputStream(new File(strFileBase + "_" + itCount + "_" + ((Estimator) listEsts.get(idxEstimator)).getName()) + "_noise_" + currentRndLevelIdx);
		...
		if (itCount == 0 && holderBaseFileEstimatorResultsPerIt.get() != null)
		{
			...
			try
			{
				FileOutputStream flOut = new FileOutputStream(new File(strFileBase + "_real"));
				BufferedOutputStream bOut = new BufferedOutputStream(flOut);
				for (int idxStep = 0; idxStep < stepCount; idxStep++)
				{
					int idxComponent = 0; 
...
//*/						
	...
	private int currentRndLevelIdx = 0;
	private int rndLevelCount = 5;
	...
	protected void calculate(RunWorkInterface runWorkInterface)
	{
		...
		NoiseControlInfo noiseCtrl = new NoiseControlInfo();
		log.debug("currentRndLevelIdx: " + currentRndLevelIdx + ", rndLevelCount: " + rndLevelCount);
		for (currentRndLevelIdx = 0; currentRndLevelIdx < rndLevelCount; currentRndLevelIdx++)
		{
			log.debug("currentRndLevelIdx: " + currentRndLevelIdx + ", rndLevelCount: " + rndLevelCount);
			noiseCtrl.noiseLevelScale = 1 + currentRndLevelIdx*0.15; 
			log.debug("noiseCtrl.noiseLevelScale: " + noiseCtrl.noiseLevelScale);
			noiseCtrl.randomReadingScale = 0; 
			noiseCtrl.randomReadingFrequency = 0; 
			estEngine.processData(itNumber, noiseCtrl, this);
		}
...
//		arrayCovars = estEngine.getEstimatesCovariance();
		...
		int countComponents = arrayRMSMeansPerComponent[0][0][0].length;
		for (int idxEstimator = 0; idxEstimator < arrayRMSMeansTotal.length; idxEstimator++)
		{
			for (int idxRndLevel = 0; idxRndLevel < rndLevelCount; idxRndLevel++)
			{
				for (int idxStep = 0; idxStep < arrayRMSMeansTotal[0][0].length; idxStep++)
				{
					for (int idxComponent = 0; idxComponent < countComponents; idxComponent++)
					{
						arrayRMSMeansPerComponent[idxEstimator][idxRndLevel][idxStep][idxComponent] /= itNumber;
						arrayRMSMeansTotal[idxEstimator][idxRndLevel][idxStep] += arrayRMSMeansPerComponent[idxEstimator][idxRndLevel][idxStep][idxComponent];
						arrayRMSMeansTotalMean[idxEstimator][idxRndLevel] += arrayRMSMeansPerComponent[idxEstimator][idxRndLevel][idxStep][idxComponent];
					}
				}
				arrayRMSMeansTotalMean[idxEstimator][idxRndLevel] /= arrayRMSMeansTotal[0][0].length;
			}
		}
		...
		if(holderBaseFileEstimatorMeans.get() != null)
		{
			...
			try
			{
				...
				for (int idxEstimator = 0; idxEstimator < arrayRMSMeansTotal.length; idxEstimator++)
				{
					for (int idxRndLevel = 0; idxRndLevel < rndLevelCount; idxRndLevel++)
					{
						FileOutputStream flOut = new FileOutputStream(new File(strFileBase + "_" + ((Estimator) listEsts.get(idxEstimator)).getName() + "_noise_" + idxRndLevel));
						BufferedOutputStream bOut = new BufferedOutputStream(flOut);
						for (int idxStep = 0; idxStep < arrayRMSMeansTotal[0][0].length; idxStep++)
						{
							StringBuffer strBuff = new StringBuffer();
							...
							for (; idxComponent < countComponents - 1; idxComponent++)
							{
								strBuff.append(arrayRMSMeansPerComponent[idxEstimator][idxRndLevel][idxStep][idxComponent]);
								strBuff.append(",");
							}
							for (; idxComponent < countComponents; idxComponent++)
							{
...
//		showLegends();
	...
	protected void showGraph()
	{
		...
		List listInfosNew = new ArrayList();
		List listEsts = estEngine.getEstimators();
		for (int i = 0; i < listEsts.size(); i++)
		{
			for (int idxRndLevel = 0; idxRndLevel < rndLevelCount; idxRndLevel++)
			{
				EstimatorRendererInfo infTmpl = (EstimatorRendererInfo) listInfos.get(i);
				EstimatorRendererInfo infoRndr = infTmpl.getCopy(infTmpl.getName() + "_noise_" + idxRndLevel);
				infoRndr.setColorFill(ColorSequence.getNonWhiteNonBlackColor(idxRndLevel + i*rndLevelCount, rndLevelCount*listEsts.size()));
				listInfosNew.add(infoRndr);

				mapEstInfos.put(infoRndr, ((Estimator) listEsts.get(i)).getName() + "_noise_" + idxRndLevel);
				mapEstRMSs.put(infoRndr, arrayRMSMeansTotalMean[i][idxRndLevel] + "");
				mapEstTimes.put(infoRndr, arrayTimes[i] + "");
			}
		}
		...
		for (int idxEst = 0; idxEst < listEsts.size(); idxEst++)
		{
			for (int idxRndLevel = 0; idxRndLevel < rndLevelCount; idxRndLevel++)
			{
				EstimatorRendererInfo estInfo = (EstimatorRendererInfo) listInfosNew.get(idxEst);
				XYSeries series = new XYSeries(((Estimator) listEsts.get(idxEst)).getName() + "_noise_" + idxRndLevel);
				for (int idxSteps = 0; idxSteps < arrayRMSMeansTotal[idxEst][idxRndLevel].length; idxSteps++) 
					series.add(idxSteps, arrayRMSMeansTotal[idxEst][idxRndLevel][idxSteps]);
...
//*/			
	...
	class RedispatchMouseEvents implements MouseListener, MouseMotionListener
	{
		...
		public void mouseReleased(MouseEvent e) {redispatchMouseEvent(e, false);}
