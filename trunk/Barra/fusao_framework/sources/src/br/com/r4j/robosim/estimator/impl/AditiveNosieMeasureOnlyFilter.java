...
public class AditiveNosieMeasureOnlyFilter extends BaseEstimator
{
	...
	{
		boolean bResults = sens.hasNewData();
		if (bResults)
		{
			AbstractDoubleVector vectReadings = new DoubleVector(sens.getDataDimension());
			sens.getData(vectReadings);
			AbstractDoubleSquareMatrix snsR = sens.getDataCovariance();
			bResults = sensModel.canProduceObservations(vectReadings, snsR);
			if (bResults)
			{
