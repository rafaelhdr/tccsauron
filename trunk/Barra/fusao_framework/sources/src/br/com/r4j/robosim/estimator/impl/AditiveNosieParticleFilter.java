...
public class AditiveNosieParticleFilter extends BaseEstimator implements ParticleFilter, Configurable
{
	...
	protected void doEstimate()
	{
		...
		AbstractDoubleVector vectEstReadings = new DoubleVector(dynModelSensor.getDataDimension(dynModel));
		dynModelSensor.getData(vectEstReadings, dynModel);
...
//											dynModel.getModelIncrementalCovariance(stateEstimate, vectEstReadings, dynModelSensor.getDataCovariance()));
											dynModelSensor.getDataCovariance(dynModel));
		if (logPF.isDebugEnabled())
		{
...
//*/
		...
		while (itSensModels.hasNext())
		{
			...
			if (bResults)
			{
				...
				bResults = snsFunc.canProduceObservations(vectReadings, snsR, stateEstimate, stateCovarEstimate);
				if (bResults)
				{
...
/*/
		...
		for (int i = 0; i < weight.dimension(); i++)
		{
			sqrOfSum += weight.getComponent(i);
			sumOfSqr += weight.getComponent(i)*weight.getComponent(i);
		}
		double ESS = sqrOfSum / sumOfSqr;
//		return (ESS < weight.dimension()*resamplingMinFactor);
		return (ESS < resamplingMinFactor);
//*/		
	}


	/** @modelguid {F6F4939A-7A6A-4CB3-86FB-9F82C1039AB0} */
