...
//		idxCurrentSonar = (idxCurrentSonar + 1)%(sonarCount + 3);
		...
		if (logSens.isDebugEnabled())
			logSens.debug("dataAvailable: " + idxCurrentSonar);
		if (this.hasNewData(null, null))
		{
	...
	{
		if (logSens.isDebugEnabled())
			logSens.debug("getData:this.hasNewData(baseModel): " + this.hasNewData(baseModel, null));

		double phoError = rnd.nextGaussian()*this.getReadingSigma();
		if (this.hasNewData(baseModel, null))
		{
			if (!bDataProcessed)
			{
				...
				idxCurrentSonar = (idxCurrentSonar + 1)%sonarCount;
				{
					double currentReading = this.findClosestWallReading(poseRobot.getX(), poseRobot.getY(), poseRobot.getTheta()) + phoError;
					if (logSens.isDebugEnabled())
						logSens.debug("getData(" + idxCurrentSonar + "): " + currentReading + ", for pose: " + poseRobot);
	...
	private double findClosestWallReading(double xR, double yR, double thetaR)
	{
		...
		double xOtherOtherSide = xSA - 0.3*this.getMaxReading()*Math.cos(thetaR + arrayThetaS[idxCurrentSonar]);
		double yOtherOtherSide = ySA - 0.3*this.getMaxReading()*Math.sin(thetaR + arrayThetaS[idxCurrentSonar]);

		double xOtherSide = xSA + 1.3*this.getMaxReading()*Math.cos(thetaR + arrayThetaS[idxCurrentSonar]);
		double yOtherSide = ySA + 1.3*this.getMaxReading()*Math.sin(thetaR + arrayThetaS[idxCurrentSonar]);
	...
	{
		return super.getReadingSigma()*noiseCtrl.noiseLevelScale;
	}
}
