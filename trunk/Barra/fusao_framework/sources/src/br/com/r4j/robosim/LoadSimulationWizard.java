			...
			for (int i = 0; i < modelList.getSize(); i++)
			{
				...
				while (itSensorModels.hasNext())
				{
					...
					if (!listSelectedSensorModelsInfos.contains(mdl))
					{
						...
						if (sns != null && !listSelectedSensorInfos.contains(sns))
...
//*/
	...
	class AddEstimator extends AbstractAction
	{
		...
		public void actionPerformed(ActionEvent e)
		{
			...
			{
				int count = 1;
				while (mapCountEst.containsKey(selectedEstimator))
				{
				...
				log.debug("AddEstimator:selectedEstimator: " + selectedEstimator.getName());
			}
		}
	}
	...
	class RemoveEstimator extends AbstractAction
	{
		...
		public void actionPerformed(ActionEvent e)
		{
			...
			{
				JOptionPane.showMessageDialog(null, 
						"Selecione algum estimador para remoção",
						"Selecione algum estimador para remoção",
						JOptionPane.ERROR_MESSAGE);
				return;
			}
	...
	private void showEstimatorSelection(JDialog diagBase)
	{
		...
		listEstimators.setSelectionMode(ListSelectionModel.SINGLE_SELECTION);
		...
		JButton btnOkNoSensor = new JButton(new OkNoSensorEstimatorSelection(diagBase, listEstimators));
		JButton btnCancel = new JButton(new CancelAction(diagBase));
...
//*/
	...
	class OkNoSensorEstimatorSelection extends AbstractAction
	{
		private JDialog diagBase = null;
		private JList listEstimators = null;


		public OkNoSensorEstimatorSelection(JDialog diagBase, JList listEstimators)
		{
			super("Ok sem Sensor"); this.diagBase = diagBase; this.listEstimators = listEstimators;
		}
...
//		pnlTop.setBorder(new EmptyBorder(5, 5, 5, 5));
		...
		gbc.gridwidth = 1; gbc.gridheight = 1;
		gbc.gridx = 0; gbc.gridy = 0;
		JLabel lblNotsel = new JLabel("Não Selecionados"); 
...
//*/
	...
	class AddSensor extends AbstractAction
	{
		...
		public void actionPerformed(ActionEvent e)
		{
			...
			{
				JOptionPane.showMessageDialog(null, 
						"Número máximo de sensores já selecionados",
						"Número máximo de sensores já selecionados",
						JOptionPane.ERROR_MESSAGE);
				return;
			}
...
//		if(idxSel == -1)
			...
			{
				JOptionPane.showMessageDialog(null, 
						"Selecione algum sensor para adição",
						"Selecione algum sensor para adição",
						JOptionPane.ERROR_MESSAGE);
				return;
			}
	...
	class RemoveSensor extends AbstractAction
	{
		...
		public void actionPerformed(ActionEvent e)
		{
			...
			{
				JOptionPane.showMessageDialog(null, 
						"Selecione algum sensor para remoção",
						"Selecione algum sensor para remoção",
						JOptionPane.ERROR_MESSAGE);
				return;
			}
	...
	class OkSensor extends AbstractAction
	{
		...
		public void actionPerformed(ActionEvent e)
		{
			...
			{
				JOptionPane.showMessageDialog(null, 
						"Número mínimo de sensores não atingido",
						"Número mínimo de sensores não atingido",
						JOptionPane.ERROR_MESSAGE);
				return;
			}
...
//				worldMap = new WorldMap();
		...
		if (bSimulationEnv)
		{
			...
			else
			{
				...
				catch(IOException ex)
				{
					log.error("erro", ex);
					JOptionPane.showMessageDialog(null, 
							"Ocorreu um problema abrindo o arquivo de ações",
							"Ocorreu um problema abrindo o arquivo de ações",
							JOptionPane.ERROR_MESSAGE);
				}
			}
		...
		else
		{
			...
			try
			{
				((RealRobotPlayer) robotPlayer).setReadingsFile((File) holderFileRealReadings.get(), (File) holderFileRealPose.get());
			}
			catch(IOException ex)
			{
				log.error("erro", ex);
...
/* MODIFICAÇÃO PARA INCLUIR VLINES ... 
...
/*/
		DynamicModelInfo dynModelInfo = estProvider.getDynamicModelInfo(bSimulationEnv);
		if (dynModelInfo instanceof MapDependent)
			((MapDependent) estProvider.getDynamicModelInfo(bSimulationEnv)).setWorldMap(worldMap);
		DynamicModel dynModel = estProvider.getDynamicModel(dynModelInfo);

		SensorInfo snsOdoInfo = dynModelInfo.getSensorInfo();
		if (!listSelectedSensorInfos.contains(snsOdoInfo))
			listSelectedSensorInfos.add(snsOdoInfo);
		Sensor snsOdo = estProvider.getSensor(snsOdoInfo);
		if (snsOdo instanceof MapDependent)
			((MapDependent) snsOdo).setWorldMap(worldMap);

		boolean bUseSensorMap = false, bUseSensorState = false;
		Iterator itSens = listSelectedSensorModelsInfos.iterator();
		while (itSens.hasNext())
		{
			SensorModelInfo snsModelInfo = (SensorModelInfo) itSens.next();
			if (snsModelInfo.getName().equals("vlinemap"))
				bUseSensorMap = true;
			if (snsModelInfo.getName().equals("vlinestate"))
				bUseSensorState = true;
		}


		VLinesModelsManager manMaster = new VLinesModelsManager();
		manMaster.configure(props, "/vlinemanager");
		manMaster.setWorldMap(worldMap);

		VLineMapSensor snsMap = manMaster.getSensorMap();
		VLineStateSensor snsState = manMaster.getSensorState();

		if (bUseSensorMap || bUseSensorState)
			robotPlayer.addSensor(snsOdo);

		if (bUseSensorMap)
			robotPlayer.addSensor(snsMap);
		if (bUseSensorState)
			robotPlayer.addSensor(snsState);

		itSens = listSelectedSensorInfos.iterator();
		while (itSens.hasNext())
		{
			...
			if ((snsOdo != sns && (bUseSensorMap || bUseSensorState)) || !(bUseSensorMap || bUseSensorState))
...
//*/
		...
		while (itEsts.hasNext())
		{
			EstimationComponentsInfo estCompsInfo = (EstimationComponentsInfo) itEsts.next();
			log.debug("EstimationComponentsInfo estCompsInfo.getName(): " + estCompsInfo.getName());
			boolean bUseSensorMapEst = false, bUseSensorStateEst = false;
			Iterator itSensModels = estCompsInfo.getSensorModels().iterator();
			while (itSensModels.hasNext())
			{
				SensorModelInfo snsModelInfoEst = (SensorModelInfo) itSensModels.next();
				if (snsModelInfoEst.getName().equals("vlinemap"))
					bUseSensorMapEst = true;
				if (snsModelInfoEst.getName().equals("vlinestate"))
					bUseSensorStateEst = true;
			}

			Estimator est = estProvider.createEstimator(estCompsInfo);
			log.debug("est.getName(): " + est.getName());
			log.debug("((BaseEstimator) est).getInstanceName(): " + ((BaseEstimator) est).getInstanceName());
			robotPlayer.addEstimator(est);

			Iterator itSensMdls = est.getSensorModels().iterator();
			while (itSensMdls.hasNext())
			{
				SensorModel snsModel = (SensorModel) itSensMdls.next();
				if (snsModel == null)
				{
					JOptionPane.showMessageDialog(null, 
							"Ocorreu um problema gerando o modelo de sensor " + snsModel.getName(),
							"Ocorreu um problema gerando o modelo de sensor " + snsModel.getName(),
							JOptionPane.ERROR_MESSAGE);
				}
				else
			...
			if (bUseSensorStateEst || bUseSensorMapEst)
			{
				VLinesModelsManager vlineManager = null;
				if (bFirst)
				{
					vlineManager = manMaster;
					bFirst = false;
				}
				else
				{
					vlineManager = new VLinesModelsManager(snsState, snsMap);
				...
				vlineManager.getDynamicModel().setSensor(snsOdo);
				vlineManager.getSensorModelMap().setSensor(snsMap);
				vlineManager.getSensorModelState().setSensor(snsState);

				est.setDynamicModel(vlineManager.getDynamicModel(), snsOdo);
				if (bUseSensorMapEst)
				{
					est.addSensorModel(vlineManager.getSensorModelMap(), snsMap);
					est.setInstanceName(est.getInstanceName() + vlineManager.getSensorModelMap().getName() + ", ");
				}
				if (bUseSensorStateEst)
				{
					est.addSensorModel(vlineManager.getSensorModelState(), snsState);
					est.setInstanceName(est.getInstanceName() + vlineManager.getSensorModelState().getName() + ", ");
				}

				robotPlayer.setDynamicModel(vlineManager.getDynamicModel());
			}
			else
			{
				est.setDynamicModel(dynModel, snsOdo);
			}
		}
