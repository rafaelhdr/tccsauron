...
import br.com.r4j.robosim.simrobot.*;
...
public class ActionFileEstimatorTest
{
	private static Log log = LogFactory.getLog(ActionFileEstimatorTest.class.getName());
	private static Log logOutput = LogFactory.getLog("output");

	
	private static ActionFileRobotPlayer realPlayer = null;
	private static SimOdoSensor odoSensor = null;
	...
	public static void main(String [] args)
	{
		try
		{
			...
			realPlayer = new ActionFileRobotPlayer();

			realPlayer.setWorldMap(worldMap);
			realPlayer.setActionFile(props.getFileProperty("/robosim/actions"));

			odoSensor = new SimOdoSensor();
			odoSensor.configure(props, "/sim_world/sensors/sensor_2");
			odoSensor.setWorldMap(worldMap);
...
//						new DoubleSquareMatrix(new double [][] {{1, 0, 0}, {0, 1, 0}, {0, 0, 0.001}}));
	...
	public ActionFileEstimatorTest()
	{
	}
