 ...
 * ------------------------------------
 *
 *  1. Add a variable to store the data to the ArGPS declaration in ArGPS.h, or
 *  a subclass,
 ...
 * commands, then it will probably work if you use the right BAUD rate. See
...
AREXPORT bool ArGPS::waitForData(unsigned long timeout)
{
  ...
  {
    if (read(0) & ReadUpdated)  // read until data is sucessfully parsed 
      return true;
    ArUtil::sleep(100);
  }
  return false;
...
void ArGPS::handleGPRMC(ArNMEAParser::Message msg)
{
  ...
  myData.qualityFlag = flag;
  myData.havePosition = true;
  myData.timeGotPosition = msg.timeParseStarted;

  // timestamp
  std::string::size_type dotpos = (*message)[1].find('.');
  time_t timeSec = atoi((*message)[1].substr(0, dotpos).c_str());
  time_t timeMSec = 0;
  if(dotpos != std::string::npos)
    timeMSec = atoi((*message)[1].substr(dotpos+1).c_str()) * 100;
  myData.GPSPositionTimestamp.setSec(timeSec);
  myData.GPSPositionTimestamp.setMSec(timeMSec);

  // speed
  myData.haveSpeed = readFloatFromStringVec(message, 7, &(myData.speed), &knotsToMPS);
